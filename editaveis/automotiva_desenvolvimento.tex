...
A resistência à flexão e da rigidez de flexão têm de ser calculado sobre um dos eixos, aquele que resultar no menor valor. Rigidez de flexão é proporcional ao produto EI e a resistência à flexão é dada pelo valor de é $\frac{S_{y}}{C}$ ( para o aço 1010 os valores são: $S_{y} = 325MPA$, E = 200MPA$)
...
\caption{Análise de deslocamento.}
\label{analise-deslocamento}
\end{figure}

O presente trabalho se baseiou principalmente no método de Von Misses, por ser de fácil e mais simples verificação. Na figura \ref{resultado-simulacao} podemos verificar, que a tensão gerada, ficou muito abaixo da tensão de escoamento do material:
Tensão máxima solicitada = 2,14MPA$ em o fator de segurança pode ser obtido divindo a tensão escoamento, pelo valor da tensão máxima gerada. O mesmo é maior que um, comprovando que o suporte projetado irá suportar as cargas impostas.
